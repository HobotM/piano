...
const createScene = async function(engine) {
    ...
    const radius = 250;
    const target = new BABYLON.Vector3(10, 50, 10);
    ...
    const keyParams = [
        {type: "white", note: "C", topWidth: 1.3, bottomWidth: 2.3, topPositionX: -0.45, wholePositionX: -14.4},
    ...
    for (let register = 1; register <= 7; register++) {
        keyParams.forEach(key => {
            buildKey(scene, keyboard, Object.assign
                ({register: register, referencePositionX: referencePositionX}, key));
        })
        referencePositionX += 2.4*7;
    }
      // Register 0
      buildKey(scene, keyboard, {type: "white", note: "A", topWidth: 1.9, bottomWidth: 2.3, 
      topPositionX: -0.20, 
      wholePositionX: -2.4, 
      register: 0,
      ...
      // Register 8
      buildKey(scene, keyboard, {type: "white", note: "C", 
      topWidth: 2.3, 
      bottomWidth: 2.3, 
      topPositionX: 0, 
    ...
    keyboard.position.y += 79;
        // Scale the entire piano
       //scaleFromPivot(piano, new BABYLON.Vector3(0, 0, 0), scale);
       const pointerToKey = new Map()
       const pianoSound = await Soundfont.instrument(new AudioContext(), 'acoustic_grand_piano'); //acoustic_grand_piano
